...
export default function Home() {
  ...
  const [calcMode, setCalcMode] = useState<"by-projector" | "by-room">("by-projector");
  ...
  const openCalculator = (mode: "by-projector" | "by-room") => { setActiveSection("calculator"); setCalcMode(mode); };
  ...
  return (
    <div className="app-shell dashboard-shell">
      ...
      <aside className="dashboard-sidebar"><a className="dashboard-logo" href="#top" aria-label="Thiết bị Văn phòng Thanh Hà, về đầu trang"><img src="https://projplanvn-7k49qncz.manus.space/manus-storage/thanh-ha-office-machines-logo_3a44d862.png" alt="Logo Thiết bị Văn phòng Thanh Hà" /><span><strong>THIẾT BỊ VĂN PHÒNG</strong><b>THANH HÀ</b></span></a><div className="sidebar-label">TƯ VẤN</div><button className={activeSection === "calculator" && calcMode === "by-room" ? "sidebar-nav active" : "sidebar-nav"} onClick={() => openCalculator("by-room")}><span className="sidebar-dot" />Tính theo phòng</button><button className={activeSection === "calculator" && calcMode === "by-projector" ? "sidebar-nav active" : "sidebar-nav"} onClick={() => openCalculator("by-projector")}><Projector size={14} />Tính theo model</button><button className="sidebar-nav" onClick={() => { setUnknownMode(true); openCalculator("by-projector"); }}><Database size={14} />Thêm model</button><button className={activeSection === "guide" ? "sidebar-nav sidebar-guide-nav active" : "sidebar-nav sidebar-guide-nav"} onClick={() => setActiveSection("guide")}><CircleHelp size={14} />Hướng dẫn</button><div className="sidebar-foot">Dữ liệu local · {PROJECTORS.length} model<br />Không thay thế manual lắp đặt</div></aside>
